...
def get_own_balance(current_user: User = Depends(get_current_user), region: Region = Depends(get_current_region), db: Session = Depends(get_db)) -> BalanceRead:
    return get_balance_summary(db, current_user, region)
